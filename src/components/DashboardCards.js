...
export default function Card({ key, title, price, type, size, text_size }) {
  ...
  return (
    <div
      onClick={clickedCard}
      className="text-sm rounded lg:flex overflow-hidden shadow-lg hover:scale-102 hover:bg-[#efefef9d] bg-white shadow-md"
    >
      <img
        className=" max-w-lg max-h-12 lg:max-h-16 object-cover"
      ...
      <div className="px-6 py-4 w-4/5">
        <div class="h-25  grid-cols-2 gap-2 content-evenly ... lg:flex">
          <div>
            <h2 className="text-tert-blue font-bold font-roboto mb-2 ">
              {title}
            </h2>
          </div>
        </div>
